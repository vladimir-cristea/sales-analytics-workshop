...
CREATE OR REPLACE TABLE vcr_serverless_catalog.shared_data.customers
...
FROM read_files('/Volumes/vcr_serverless_catalog/shared_data/data/clean/customers/', format => 'json',
  schemaHints => 'customer_id INT, join_date DATE');

-- COMMAND ----------

CREATE OR REPLACE TABLE vcr_serverless_catalog.shared_data.products
...
FROM read_files('/Volumes/vcr_serverless_catalog/shared_data/data/clean/products/', format => 'json',
  schemaHints => 'list_price DECIMAL(10,2), cost DECIMAL(10,2), launch_date DATE');

-- COMMAND ----------

CREATE OR REPLACE TABLE vcr_serverless_catalog.shared_data.orders
...
FROM read_files('/Volumes/vcr_serverless_catalog/shared_data/data/clean/orders/', format => 'json',
  schemaHints => 'order_id INT, customer_id INT, quantity INT, order_date DATE, unit_price DECIMAL(10,2), discount_pct DECIMAL(5,2)');

-- COMMAND ----------

CREATE OR REPLACE TABLE vcr_serverless_catalog.shared_data.product_performance_summary
...
FROM vcr_serverless_catalog.shared_data.products p
LEFT JOIN vcr_serverless_catalog.shared_data.orders o ON p.product_id = o.product_id
GROUP BY p.product_id, p.product_name, p.category, p.list_price, p.cost;

-- COMMAND ----------

CREATE OR REPLACE TABLE vcr_serverless_catalog.shared_data.monthly_sales_summary
...
JOIN vcr_serverless_catalog.shared_data.customers c ON o.customer_id = c.customer_id
