-- ---------------------------------------------------------------------------
-- Practical 3 - Lakebase  •  STEP 3 (participant): point-lookup exercises
-- ---------------------------------------------------------------------------
...
-- NEVER run live against the transactional source. You precomputed it in the
-- lakehouse and synced it here as a plain Postgres table keyed by customer_id -
-- single-digit-millisecond point lookups. "Look, it's real Postgres."
--
-- Run this against YOUR branch after step 2 has synced. The synced table lands
-- in your scratch schema as <your-ws-schema>.customer_scorecard_synced. Set the
-- search_path to it so the queries below need no schema prefix; edit the line to
-- your own ws_<user> schema name.
SET search_path TO "ws_REPLACE_WITH_YOUR_SCHEMA", public;
--
-- The synced table is range-partitioned on the PK, so EXPLAIN reads
-- `Seq Scan on partition_N` by default at this row count (~0.2 ms; force the
-- index with `SET enable_seqscan=off` → `Index Scan … on partition_N`). Either
-- way the point lookup is sub-millisecond; the index proves itself at scale.
-- ---------------------------------------------------------------------------
...
FROM   customer_scorecard_synced
...
EXPLAIN ANALYZE SELECT * FROM customer_scorecard_synced WHERE customer_id = 42;
RESET enable_seqscan;
-- With the index forced you get an Index Scan on the PK … sub-millisecond.

-- 3) Precomputed at-risk flag - serve a churn worklist instantly.
