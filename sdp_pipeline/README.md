...
[`reference_solution/`](reference_solution/) holds the complete reference pipeline (three
SQL files: `bronze.sql`, `silver.sql`, `gold.sql`) plus a README with the table definitions,
the silver/gold split, run instructions, and the expected row counts. It is the facilitator's
answer key for validating participant output - **not** a participant handout. See
[`reference_solution/README.md`](reference_solution/README.md).
