...
psql "$PROD_URI" -c "SELECT count(*) AS rows_after_oops FROM public.customer_scorecard;"  # tested: 50
...
psql "$PGURI" -c "SELECT count(*) AS recovered_rows, max(customer_id) AS max_id FROM public.customer_scorecard;"  # tested: 70 / 50->70
